...
//
// LOCAL CHANGE: the upstream component animates unconditionally. This site
// treats prefers-reduced-motion as a hard requirement, so the render loop is
// gated on it below — see `prefersReducedMotion`. Everything else is upstream;
// keep local edits marked so the next version bump is a readable diff.
...
const UNIFORMS = {
  colors: [[0.0196078431372549,0.027450980392156862,0.050980392156862744],[0.07058823529411765,0.3803921568627451,0.6274509803921569],[0.20784313725490197,0.7686274509803922,0.9098039215686274],[0.9490196078431372,0.984313725490196,1],[0.9490196078431372,0.984313725490196,1],[0.9490196078431372,0.984313725490196,1],[0.9490196078431372,0.984313725490196,1],[0.9490196078431372,0.984313725490196,1]] as [number, number, number][],
  colorCount: 4,
  scale: 1.260,
  intensity: 0.350,
  paramA: 0.280,
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // LOCAL CHANGE — reduced motion. When the user asks for less motion the
    // shader still paints one frame, so the border is there as a static piece
    // of composition; only the pulse stops. `timeAnimated` is what keeps the
    // rAF loop alive, so gating it is the whole fix.
    const motionQuery = window.matchMedia("(prefers-reduced-motion: reduce)")
    let timeAnimated =
      Math.abs(UNIFORMS.timeScale) > 0.0001 && !motionQuery.matches
    const onMotionPreferenceChange = () => {
      timeAnimated = Math.abs(UNIFORMS.timeScale) > 0.0001 && !motionQuery.matches
      requestRender()
    }
    ...
    // LOCAL CHANGE — was `function render(...)`. A hoisted declaration sits
    // above the `if (!canvas) return` / `if (!gl) return` guards, so TypeScript
    // discards the non-null narrowing inside it and strict mode rejects every
    // `canvas.` and `gl.` reference. An arrow defined after the guards keeps
    // the narrowing. Behaviour is identical: `requestRender` only reads
    // `render` when called, which is always after this assignment.
